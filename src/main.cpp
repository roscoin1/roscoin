...
unsigned int nTargetSpacingNew = 6 * 60;     // 360 seconds
...
const string strMessageMagic = "Roscoin Signed Message:\n";
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
	...
	if (pindexBest->nHeight >= 75000) {
		int64_t nInterval = nTargetTimespan / nTargetSpacingNew;
		bnNew *= ((nInterval - 1) * nTargetSpacingNew + nActualSpacing + nActualSpacing);
		bnNew /= ((nInterval + 1) * nTargetSpacingNew);
	}	
